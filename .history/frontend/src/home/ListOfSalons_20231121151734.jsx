...
const sortOptions = [
  ...
  export default function ListOfSalons({search}) {
    ...
    useEffect(() => {
      setResolut(salons.filter(item=>item.address.city.cityName.includes(search.cityName != "" ? search.cityName : salons.length)).length+1)
      console.log(result)
    })
    const searchCity = ()=>
    {
      return salons.filter(item=>item.address.city.cityName.includes(search.cityName ? search.cityName : ""))
    }
