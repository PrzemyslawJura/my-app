...
const sortOptions = [
  ...
  export default function ListOfSalons({search}) {
    ...
    useEffect(() => {
      ...
      {
        setResolut((salons.filter(item=>item.address.city.cityName.includes(search)).length))
      }
    ...
    const searchCity = ()=>
    {
      return salons.filter(item=>item.address.city.cityName.includes(search ? search : ""))
    }
