...
const sortOptions = [
  ...
  export default function ListOfSalons({search}) {
    ...
    const searchCity = ()=>
    {
        if (search == "")
        {
          return console.log(salons.filter(item=>item.address.city.cityName.includes("")));
        }
        return console.log(salons.filter(item=>item.address.city.cityName.includes(search.cityName)));
    }
